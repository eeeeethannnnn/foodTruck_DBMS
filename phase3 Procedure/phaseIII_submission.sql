...
DROP VIEW IF EXISTS staion_foodtruck_count;
CREATE VIEW staion_foodtruck_count AS
SELECT stationName, count(*) AS foodtruck_count
FROM FoodTruck
GROUP BY stationName;
-- 
-- DROP VIEW IF EXISTS station_remaining_capacity;
-- CREATE VIEW station_remaining_capacity AS
-- SELECT Station.stationName, Station.capacity - staion_foodtruck_count.foodtruck_count as remaining_capacity
-- FROM Station
-- INNER JOIN staion_foodtruck_count
-- ON Station.stationName =  staion_foodtruck_count.stationName;

DROP VIEW IF EXISTS station_remaining_capacity;
CREATE VIEW station_remaining_capacity AS
SELECT Station.stationName, Station.capacity - COUNT(FoodTruck.foodTruckName) as remaining_capacity
FROM Station
LEFT JOIN FoodTruck
ON Station.stationName =  FoodTruck.stationName
...
-- 		WHERE username = i_username OR (firstName = i_firstname AND lastname = i_lastname);
    -- place your code/solution here
    IF NOT EXISTS (SELECT username FROM `User` 
		WHERE username = i_username OR (firstName = i_firstname AND lastname = i_lastname))
		THEN 
			IF i_balance is not null and i_balance <> "" and i_balance > 0 and LENGTH(i_password) >= 8 THEN 
			...
			IF i_email is not null and i_email <> "" and LENGTH(i_password) >= 8 THEN
				IF NOT EXISTS(SELECT username FROM `User` WHERE username = i_username OR (firstName = i_firstname AND lastname = i_lastname)) THEN
					INSERT INTO `User` VALUES(i_username, md5(i_password), i_firstname, i_lastname);
				END IF;
...
BEGIN
    ...
    SELECT FoodTruck.foodTruckName, FoodTruck.stationName, (Station.capacity - staion_foodtruck_count.foodtruck_count) AS remaining_capacity, COUNT(DISTINCT STAFF.username), COUNT(DISTINCT foodName)
    FROM FoodTruck
    INNER JOIN STATION
    ...
    INNER JOIN staion_foodtruck_count
    ON FoodTruck.stationName =staion_foodtruck_count.stationName
    WHERE 
	(i_managerUsername = FoodTruck.managerUsername) AND
    (FoodTruck.foodTruckName LIKE CONCAT('%', i_foodTruckName, '%')OR i_foodTruckName = "" OR i_foodTruckName is NULL)  AND
    (i_stationName = FoodTruck.stationName OR i_stationName = "" OR i_stationName is NULL) AND
    ((i_hasRemainingCapacity = TRUE AND Station.capacity - staion_foodtruck_count.foodtruck_count > 0) OR (i_hasRemainingCapacity = FALSE))
...
-- 	DECLARE remaining_capacity INT DEFAULT 0;
--     
-- -- SELECT Station.capacity - staion_foodtruck_count.foodtruck_count
-- -- --     INTO remaining_capacity
-- -- --     FROM Station
-- -- --     INNER JOIN staion_foodtruck_count
-- -- --     ON Station.stationName =  station_foodtruck_count.stationName
-- -- --     WHERE Station.stationName =  i_stationName;
-- 
-- 
-- --         SELECT station_remaining_capacity.remaining_capacity
-- --         INTO remaining_capacity
-- --         FROM station_remaining_capacity
-- --         WHERE station_remaining_capacity.stationName = i_stationName;
--         
...
BEGIN
     ...
     FROM STAFF
     INNER JOIN `User`
	 ON`User`.username = STAFF.username
     WHERE ((STAFF.foodTruckName is NULL) or (STAFF.foodTruckName = ""));
...
BEGIN
    ...
    INSERT INTO MenuItem VALUES(i_price, i_foodTruckName,  i_foodName);
...
BEGIN
    ...
    CREATE TABLE mn_filter_summary_result(foodTruckName VARCHAR(50), totalOrder int, totalRevenue DECIMAL(6,2), totalCustomer int);
...
--         (i_minDate IS NULL OR i_minDate = "" OR Orders.date >= i_minDate) AND (i_maxDate IS NULL OR i_maxDate = "" OR Orders.date <= i_maxDate)
--         GROUP BY FoodTruck.foodTruckName
--     )  AS T
--     ORDER BY
--         CASE WHEN ((i_sortedBy = NULL OR i_sortedBy = "") AND (i_sortedDirection = "ASC" OR i_sortedDirection = NULL)) THEN foodTruckName END ASC,
--         CASE WHEN ((i_sortedBy = NULL OR i_sortedBy = "") AND i_sortedDirection = "DESC") THEN foodTruckName END DESC,
...
--         CASE WHEN i_sortedBy = "totalCustomer" AND i_sortedDirection = "DESC" THEN totalCustomer END DESC;
    ...
    (   SELECT FoodTruck.foodTruckName as foodTruckName, COUNT(DISTINCT (Orders.orderID)) as totalOrder, SUM(MenuItem.price * OrderDetail.purchaseQuantity) as totalRevenue, COUNT(DISTINCT customerUsername) as totalCustomer
        FROM FoodTruck
        LEFT JOIN OrderDetail ON OrderDetail.foodTruckName = FoodTruck.foodTruckName 
...
BEGIN
    ...
    SELECT Station.stationName as stationName_, Station.buildingName as buildingName_, GROUP_CONCAT(DISTINCT(FoodTruck.foodTruckName)  SEPARATOR ','), GROUP_CONCAT(DISTINCT(menuItem.foodName) SEPARATOR ',')
	FROM Station
	INNER JOIN FoodTruck
	ON FoodTruck.StationName = Station.stationName
	INNER JOIN BuildingTag ON Station.buildingName  = BuildingTag.buildingName 
	INNER JOIN menuItem ON FoodTruck.foodTruckName = MenuItem.foodTruckName
	WHERE FoodTruck.foodTruckName IN
    -- Select food truck 
	(SELECT menuItem.foodTruckName
	FROM menuItem 
	WHERE  (i_foodName is NULL OR i_foodName  = "" OR menuItem.foodName LIKE CONCAT('%', i_foodName, '%'))
    AND (i_foodTruckName is NULL OR i_foodTruckName = "" OR menuItem.foodtruckname LIKE CONCAT('%', i_foodTruckName, '%')))
	
    
	AND  (i_stationName is NULL OR i_stationName = "" OR i_stationName = Station.stationName)
	AND  (i_buildingName is NULL OR i_buildingName = "" OR i_buildingName = Station.buildingName) 
	AND (i_buildingTag is NULL OR i_buildingTag = "" OR BuildingTag.tag LIKE CONCAT('%', i_buildingTag, '%'))
...
-- call cus_select_location("4400_thebestclass", "Skiles Walkway");
...
BEGIN
    ...
    SELECT Station.stationName, Building.buildingName , Tag.tag, Building.`description`, Customer.balance
    FROM Customer
    LEFT JOIN Station 
    ON Customer.stationName = Station.stationName
    LEFT JOIN Building
    ON Station.buildingName = Building.buildingName
	LEFT JOIN
    (SELECT Building.buildingName, GROUP_CONCAT(tag SEPARATOR ',') as tag
    FROM Building
    LEFT JOIN BuildingTag
    ON Building.buildingName = BuildingTag.buildingName
    GROUP BY buildingName
    ) as Tag
    ON Building.buildingName = Tag.buildingName
...
BEGIN
    ...
    SELECT FoodTruck.foodTruckName, CONCAT(firstName , ' ' , lastName), Menu.foodName
    FROM Customer
    LEFT JOIN Station 
    ON Customer.stationName = Station.stationName
    LEFT JOIN FoodTruck
    ON Station.stationName = FoodTruck.stationName
    LEFT JOIN `User`
    ON `User`.username = FoodTruck.managerUsername
	LEFT JOIN
    (SELECT FoodTruck.foodTruckName, GROUP_CONCAT(foodName SEPARATOR ',') as foodName
    FROM FoodTruck
    LEFT JOIN MenuItem
    ON FoodTruck.foodTruckName = MenuItem.foodTruckName
    GROUP BY FoodTruck.foodTruckName
    ) as Menu
    ON FoodTruck.foodTruckName = Menu.foodTruckName
    WHERE i_customerUsername = Customer.username;
...
BEGIN
     ...
     -- place your code/solution here
    DECLARE food_price DECIMAL(6,2) DEFAULT 0.0;
	DECLARE customer_balance DECIMAL(6,2) DEFAULT 0.0;
    ...
    SET @food_price = food_price *  i_purchaseQuantity;

    SELECT Customer.balance 
    INTO customer_balance
    FROM Customer 
	INNER JOIN Orders
    ON Customer.username = Orders.customerUsername
    WHERE Orders.orderID = i_orderId;

    IF food_price < customer_balance
    THEN 
-- 		IF EXISTS(SELECT OrderDetail.orderID FROM OrderDetail WHERE (OrderDetail.foodName =  i_foodName AND OrderDetail.foodTruckName = i_foodTruckName AND OrderDetail.orderID = i_orderId))
-- 		THEN
-- 			UPDATE OrderDetail
-- 			SET OrderDetail.purchaseQuantity = OrderDetail.purchaseQuantity + i_purchaseQuantity
--             WHERE OrderDetail.foodName =  i_foodName AND OrderDetail.foodTruckName = i_foodTruckName AND OrderDetail.orderID = i_orderId;
            
			
		IF NOT EXISTS(SELECT OrderDetail.orderID FROM OrderDetail WHERE (OrderDetail.foodName =  i_foodName AND OrderDetail.foodTruckName = i_foodTruckName AND OrderDetail.orderID = i_orderId))
        THEN
			INSERT INTO OrderDetail 
			VALUES(i_orderId, i_foodTruckName, i_foodName, i_purchaseQuantity);
			
			UPDATE  Customer 
			SET balance  = balance - food_price
			WHERE username = (
...
BEGIN
    ...
    SELECT Orders.`date`, Orders.orderID,  sum(MenuItem.price * OrderDetail.purchaseQuantity),GROUP_CONCAT(DISTINCT OrderDetail.foodName), sum(OrderDetail.purchaseQuantity)
	FROM Orders 
	LEFT JOIN OrderDetail
	ON Orders.orderID = OrderDetail.orderID
	LEFT JOIN MenuItem
	ON OrderDetail.foodName = MenuItem.foodName and OrderDetail.foodTruckName = MenuItem.foodTruckName
	LEFT JOIN Customer
	ON Orders.customerUsername = Customer.username
