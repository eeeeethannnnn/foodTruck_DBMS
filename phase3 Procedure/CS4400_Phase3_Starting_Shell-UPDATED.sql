...
BEGIN
    ...
    -- Does it need to check i_balance <> ""?
    -- Customer
...
BEGIN
    ...
    SELECT FoodTruck.foodTruckName, FoodTruck.stationName, Station.capacity, COUNT(DISTINCT STAFF.username), COUNT(DISTINCT foodName)
    FROM FoodTruck
    INNER JOIN STATION
...
-- This one doesn't check for manager(zhr)
...
-- Query #extra: mn_view_foodTruck_staff [Screen #13 Manager Update Food Truck]
-- Get availabe stations

DROP PROCEDURE IF EXISTS mn_view_foodTruck_station;
DELIMITER //
CREATE PROCEDURE mn_view_foodTruck_station(i_foodTruckName VARCHAR(50))
BEGIN
    DROP TABLE IF EXISTS mn_view_foodTruck_staff_result;
    CREATE TABLE mn_view_foodTruck_staff_result(stationName varchar(100))
    SELECT stationName
    FROM Station
    WHERE (capacity > 0);
...
BEGIN
	DECLARE i_oldStationCapacity INT DEFAULT 0;
    DECLARE i_newStationCapacity INT DEFAULT 0;
    DECLARE i_oldStationName Char(55);
    
    SELECT capacity, Station.stationName
    INTO i_oldStationCapacity, i_oldStationName
    FROM Station
    LEFT JOIN FoodTruck
    ...
    SELECT capacity
    INTO i_newStationCapacity
    FROM Station
    WHERE Station.stationName = i_stationName;
    
    UPDATE Station
    SET capacity = i_oldStationCapacity + 1
    WHERE Station.stationName = i_oldStationName;
	
    UPDATE Station
    SET capacity = i_newStationCapacity - 1
    WHERE Station.stationName = i_stationName;
...
--         SELECT Station.stationName,  Station.buildingName, tags, Building.description, Customer.balance
--         FROM Station
--         LEFT JOIN Building 
--         ON Station.buildingName = Building.buildingName
--         LEFT JOIN 
--         (SELECT B.buildingName, GROUP_CONCAT(BT.tag) as tags 
		-- FROM Building as B 
		-- LEFT JOIN BuildingTag as BT 
        -- on B.buildingName = BT.buildingName 
        -- GROUP BY B.buildingName) 
        -- as buildingInfo
--         ON buildingInfo.buildingName = Building.buildingName
--         LEFT JOIN Customer
--         ON Station.stationName = Customer.stationName
...
--     SELECT FoodTruck.foodTruckName, FoodTruck.managerUsername, foodTruckMenu.foods
--     FROM 
--     Customer 
...
--     INNER JOIN 
--     (SELECT FoodTruck.foodTruckName as foodTruckName, GROUP_CONCAT(MenuItem.foodName) as foods
--     FROM FoodTruck 
--     LEFT JOIN MenuItem on FoodTruck.foodTruckName = MenuItem.foodTruckName
--     GROUP BY FoodTruck.foodTruckName) as foodTruckMenu

--     on FoodTruck.foodTruckName = foodTruckMenu.foodTruckName

--     WHERE Customer.username = i_customerUsername;
	INSERT INTO cus_current_information_foodTruck_result
    SELECT FoodTruck.foodTruckName, FoodTruck.managerUsername, Menu.foodName
    FROM Customer
    LEFT JOIN Station 
    ON Customer.stationName = Station.stationName
    LEFT JOIN FoodTruck
    ON Station.stationName = FoodTruck.stationName
	LEFT JOIN
    (SELECT FoodTruck.foodTruckName, GROUP_CONCAT(foodName) as foodName
    FROM FoodTruck
    LEFT JOIN MenuItem
    ON FoodTruck.foodTruckName = MenuItem.foodTruckName
    GROUP BY FoodTruck.foodTruckName
...
-- call cus_order_information( "mKJerrY", "GoodOnAStudentBudget");
-- Query #extra: cus_order_information [Screen #18 Customer Order]
DROP PROCEDURE IF EXISTS cus_order_information;
DELIMITER //
CREATE PROCEDURE cus_order_information(IN i_customerUsername VARCHAR(55), i_foodTruckName VARCHAR(55))
BEGIN
	SELECT MenuItem.foodName, MenuItem.price
    FROM Customer
    LEFT JOIN Station 
    ON Customer.stationName= Station.stationName
    LEFT JOIN FoodTruck
	...
	LEFT JOIN MenuItem
    ON FoodTruck.foodTruckName = MenuItem.foodTruckName
    WHERE i_customerUsername = Customer.username and i_foodTruckName = FoodTruck.foodTruckName;
...
BEGIN
	...
	-- calcualte total_price 
    SET @total_price = food_price *  i_purchaseQuantity;
    
	-- read customer_balance
    ...
    -- if cutomer has enough balance
    -- then update OrderDetail and cutomer's balance
...
BEGIN
    ...
    SELECT Orders.`date`, Orders.orderID,  sum(MenuItem.price), group_concat(OrderDetail.foodName), sum(OrderDetail.purchaseQuantity)
	FROM Orders 
